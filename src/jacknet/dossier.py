...
def dossier_for_ip(ip: str) -> dict | None:
    ...
    with connect() as con:
        ...
        if not row:
            candidates=con.execute("""SELECT d.device_id,d.canonical_mac,d.first_seen,d.last_seen,d.user_label,d.manufacturer,d.model,d.device_type,d.confidence
                FROM devices d JOIN device_network_addresses a ON a.device_id=d.device_id WHERE a.ip=? ORDER BY a.last_seen DESC""",(ip,)).fetchall()
        ...
        endpoints=con.execute("""SELECT e.endpoint,e.first_seen,e.last_seen,e.hits,e.protocol,n.name
            FROM device_endpoints e LEFT JOIN networks n ON n.network_id=e.network_id WHERE e.device_id=? ORDER BY e.hits DESC,e.last_seen DESC LIMIT 100""",(did,)).fetchall()
...
def all_devices() -> list[dict]:
    ...
    return [{"device_id":r[0],"mac":r[1],"mac_type":_mac_type(r[1]),"label":r[2],"manufacturer":r[3],"model":r[4],"device_type":r[5],"confidence":r[6],"first_seen":r[7],"last_seen":r[8],"current_ip":r[9],"address_count":r[10],"network_count":r[11],"traffic_packets":r[12],"current_network":current_network.name,"current_network_id":current_network_id} for r in rows]
