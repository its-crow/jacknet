...
def device_features(d: Device) -> set[tuple[str,str]]:
    ...
    if d.hostname:
        ...
        # Prefix strips changing numeric/hex suffixes: HPB00CD10C7383 -> hpb
...
def confirm(ip: str, manufacturer=None, model=None, device_type=None, correction=False):
    ...
    if device_id is None:
        raise LookupError(f"The latest observation for {ip} has no MAC address; JackNet will not persist identity by IP alone.")
...
def learn():
    """Mine repeated high-confidence observations + user labels into conservative reusable fingerprints."""
    migrate()
    examples=[]
    with connect() as con:
        ...
        for did,payload,model,typ,conf,user in rows:
            ...
            # Self-learning is allowed only from strong conclusions; user confirmations always qualify.
            if not user and (conf or 0) < 70: continue
            try: d=_device_from_payload(payload)
            except Exception: continue
            examples.append((did,model,typ,bool(user),device_features(d)))
        ...
        for (model,typ), exs in groups.items():
            ...
            # Features must recur, unless explicitly confirmed by the user.
            ...
            for ft,val,n in common:
                # User-backed learned rules matter more; self-learned rules stay modest.
...
def apply_learned(device: Device) -> Device:
    ...
    # Require either two learned signals, or one user-strength signal.
    ms=matches[key]
