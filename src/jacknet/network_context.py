...
def _windows_wifi(interface: str | None) -> tuple[str | None, str | None]:
    ...
    for block in blocks:
        ...
        if interface and name and name.group(1).strip().lower() != interface.lower():
            continue
        ssid = re.search(r"^\s*SSID\s*:\s*(.+)$", block, re.I | re.M)
        bssid = re.search(r"^\s*BSSID\s*:\s*([0-9a-f:-]+)$", block, re.I | re.M)
        if ssid:
            return ssid.group(1).strip(), bssid.group(1).strip().lower() if bssid else None
    return None, None
...
def _gateway_mac(gateway_ip: str | None) -> str | None:
    if not gateway_ip:
        return None
    # A ping gives Windows a chance to populate the neighbor cache.
...
def detect_network() -> NetworkContext:
    ...
    if os.name == "nt":
        ...
        interface = info.get("InterfaceAlias")
        desc = info.get("InterfaceDescription")
        ipv4 = info.get("IPv4")
        gateway = info.get("Gateway")
        try:
            if ipv4 and info.get("PrefixLength") is not None:
                cidr = str(ipaddress.ip_network(f"{ipv4}/{int(info['PrefixLength'])}", strict=False))
        except Exception:
            pass
    ...
    if not cidr:
        for name, addrs in psutil.net_if_addrs().items():
            ...
            if ipv4_addr and ipv4_addr.netmask:
                ...
                try:
                    cidr = str(ipaddress.ip_network(f"{ipv4_addr.address}/{ipv4_addr.netmask}", strict=False))
                except ValueError:
                    pass
                break

    # Stable logical-network identity: router/L2 identity + subnet + SSID when available.
    # Do not include this computer's NIC/MAC, because JackNet may run from another sensor later.
    identity = "|".join([gateway_mac or "", gateway or "", cidr or "", ssid or ""]).lower()
    if not identity.strip("|"):
        identity = f"unknown|{interface or 'network'}"
...
def ensure_network(context: NetworkContext | None = None) -> tuple[int, NetworkContext]:
    ...
    with connect() as con:
        ...
        if row:
            ...
            con.execute(
                """UPDATE networks SET last_seen=?,name=?,cidr=?,gateway_ip=?,gateway_mac=?,ssid=?,interface=?,interface_description=?
                   WHERE network_id=?""",
                (now, ctx.name, ctx.cidr, ctx.gateway_ip, ctx.gateway_mac, ctx.ssid, ctx.interface, ctx.interface_description, network_id),
            )
        else:
            cur = con.execute(
                """INSERT INTO networks(network_key,name,cidr,gateway_ip,gateway_mac,ssid,interface,interface_description,first_seen,last_seen)
                   VALUES(?,?,?,?,?,?,?,?,?,?)""",
