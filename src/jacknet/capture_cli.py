...
def _friendly_capture_name(description: str) -> str:
    matches = re.findall(r"\(([^()]*)\)", description); return matches[-1].strip() if matches else description.strip()
def _is_real_lan_interface(description: str) -> bool:
    friendly=_friendly_capture_name(description);text=f"{description} {friendly}".lower()
    if any(token in text for token in _BLOCKED_CAPTURE_NAMES):return False
    normalized=friendly.lower().replace("_","-");return normalized in ("wi-fi","wifi","ethernet") or normalized.startswith(("wi-fi ","wifi ","ethernet "))
def _windows_adapter_info():
    ...
    try:
        p=subprocess.run(["powershell.exe","-NoProfile","-NonInteractive","-Command",script],capture_output=True,text=True,encoding="utf-8",errors="replace",timeout=10,check=False)
        if p.returncode or not p.stdout.strip():return {}
        data=json.loads(p.stdout);data=[data] if isinstance(data,dict) else data
    ...
    except (OSError,subprocess.TimeoutExpired,json.JSONDecodeError):return {}
def _capture_candidates():
    ...
    for num,desc in list_interfaces():
        ...
        rows.append({"id":num,"capture":desc,"name":name,"description":info.get("InterfaceDescription","") or desc,"status":info.get("Status",""),"link_speed":info.get("LinkSpeed",""),"mac":info.get("MacAddress",""),"guid":info.get("InterfaceGuid","")})
    rows.sort(key=lambda r:(r["status"].lower()!="up",r["name"].lower()));return rows
def _run_diag(cmd,timeout=12):
    try:
...
def _capture_backend_diagnostics(interface,duration=3):
    ...
    rows=[];s,n=_windows_npcap_service();rows.append(("Npcap service",s,n));ok,x=_linktypes(exe,interface);rows.append(("Managed link types","READY" if ok else "WARN",x or "No link-layer types reported"));p,d=_packet_probe(exe,interface,duration);rows.append(("Managed capture","READY" if p else "FAIL",f"{p} packets"+(f" • {d}" if d else "")));p,d=_packet_probe(exe,interface,duration,no_promisc=True);rows.append(("Managed no-promisc","READY" if p else "FAIL",f"{p} packets"+(f" • {d}" if d else "")));ok,x=_linktypes(exe,interface,True);rows.append(("Monitor link types","READY" if ok else "INFO",x or "Monitor mode not exposed by this adapter/driver"))
    if ok:
...
def interfaces_cmd():
    ...
    rows=_capture_candidates();t=Table(title="JACKNET / NETWORK CAPTURE INTERFACES");[t.add_column(c,overflow="fold") for c in ("ID","Adapter","Hardware","Status","Link speed","MAC")]
...
def diagnose_cmd(interface:str=typer.Option(...,"-i","--interface"),duration:int=typer.Option(3,"-d","--duration",min=1,max=10)):
    ...
    adapter=next((r for r in candidates if r["id"]==interface),None);rows=_capture_backend_diagnostics(interface,duration);t=Table(title=f"JACKNET / CAPTURE DIAGNOSTICS • {adapter['name'] if adapter else interface} • {adapter['description'] if adapter else ''}");t.add_column("Check");t.add_column("Status");t.add_column("Details",overflow="fold")
...
def probe_cmd(duration:int=typer.Option(3,"-d","--duration",min=1,max=10)):
    ...
    rows=_capture_candidates();t=Table(title=f"JACKNET / NETWORK CAPTURE PROBE • {duration}s per interface");[t.add_column(c,overflow="fold") for c in ("ID","Adapter","Hardware","Status","Packets","Capture")];best=None;best_packets=-1
    for r in rows:
...
def live_cmd(interface:str=typer.Option(...,"-i","--interface"),duration:int=typer.Option(60,"-d","--duration",min=1),monitor:bool=typer.Option(False,"--monitor"),output:Path|None=typer.Option(None,"-o","--output")):
    ...
    out=output or paths()["cache"]/f"capture-{datetime.now().strftime('%Y%m%d-%H%M%S')}.pcapng";console.print(f"[bold]JACKNET / CAPTURE[/] interface [cyan]{interface}[/] • {duration}s");live_capture(interface,out,duration,monitor);stats=ingest_capture(out,source="live",interface=interface)
    if stats["packets"]==0:raise RuntimeError(f"Capture completed but interface {interface} produced 0 packets")
    learned=run_learning();console.print(Panel(f"Network: {stats['network']} (id {stats['network_id']})\nPackets: {stats['packets']:,}\nFull decodes stored: {stats.get('decoded_packets',0):,}\nArtifacts extracted: {stats.get('artifacts',0):,}\nGraph relationships: {stats.get('relationships',0):,}\nDevices linked: {stats['devices']}\nCapture: {out}\nFingerprints promoted: {learned['promoted']}",title="JACKNET / CAPTURE COMPLETE"))
...
def database_report_cmd(output:Path|None=typer.Option(None,"-o","--output"),json_out:bool=typer.Option(False,"--json")):
    ...
    if json_out:typer.echo(json.dumps(rows,indent=2))
        ...
        current=rows[0]["current_network"] if rows else "current network";t=Table(title=f"JACKNET / LEARNED NETWORK DATABASE • {current}")
        for c in ("ID","Network","Current IP","MAC","MAC type","Identity","Type","Confidence","Traffic","Addr hist","Networks","Last seen"):t.add_column(c,overflow="fold")
        for r in rows:t.add_row(str(r['device_id']),r['current_network'],r['current_ip'] or '—',r['mac'] or '—',r['mac_type'],r['label'] or r['model'] or 'Unknown',r['device_type'] or 'unknown',f"{r['confidence'] or 0}%",str(r['traffic_packets']),str(r['address_count']),str(r['network_count']),r['last_seen'])
        console.print(t)
