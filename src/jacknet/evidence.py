...
def _lookup_device(con,network_id,mac,ip):
 if _usable_mac(mac):
 ...
 if _local_ip(ip):
  row=con.execute("SELECT device_id FROM device_network_addresses WHERE network_id=? AND ip=? ORDER BY last_seen DESC LIMIT 1",(network_id,ip)).fetchone();return int(row[0]) if row else None
 return None
...
def _touch_relationship(con,network_id,did,relation,source_type,source_value,target_type,target_value,when,protocol):
 row=con.execute("SELECT id,hits FROM network_relationships WHERE network_id IS ? AND device_id IS ? AND relation=? AND source_type=? AND source_value=? AND target_type=? AND target_value=?",(network_id,did,relation,source_type,source_value,target_type,target_value)).fetchone()
 ...
 else:con.execute("INSERT INTO network_relationships(device_id,relation,source_type,source_value,target_type,target_value,first_seen,last_seen,hits,protocol,metadata,network_id) VALUES(?,?,?,?,?,?,?,?,1,?,NULL,?)",(did,relation,source_type,source_value,target_type,target_value,when,when,protocol,network_id))
...
def _artifact_owner(typ,src_ip,dst_ip,src_did,dst_did):
 if typ in {"domain_answer","tls_cert_subject"}:return dst_did if _local_ip(dst_ip) else src_did
...
def ingest_full_decode(path:Path,session_id:int,tshark:str):
 ...
 with connect() as con:
  srow=con.execute("SELECT network_id FROM capture_sessions WHERE id=?",(session_id,)).fetchone();network_id=int(srow[0]) if srow and srow[0] is not None else None
  for raw in proc.stdout:
   ...
   packet_number+=1;flat=_flatten(obj.get("layers",{}));observed_at=_iso_epoch(_first(flat,"frame.time_epoch"));stack=_first(flat,"frame.protocols");src_mac=_first(flat,"eth.src","wlan.sa");dst_mac=_first(flat,"eth.dst","wlan.da");src_ip=_first(flat,"ip.src","ipv6.src");dst_ip=_first(flat,"ip.dst","ipv6.dst");src_did=_lookup_device(con,network_id,src_mac,src_ip);dst_did=_lookup_device(con,network_id,dst_mac,dst_ip)
   ...
   for typ,value in _artifact_pairs(flat,stack):
    owner=_artifact_owner(typ,src_ip,dst_ip,src_did,dst_did);con.execute("INSERT INTO network_artifacts(session_id,device_id,observed_at,artifact_type,artifact_value,source,protocol,metadata,network_id) VALUES(?,?,?,?,?,'tshark-decode',?,NULL,?)",(session_id,owner,when,typ,value,proto,network_id));artifacts+=1
    if owner is not None and typ in {"domain_query","domain_answer","tls_sni","http_host","mdns_name","service"}:
     target="domain" if typ in {"domain_query","domain_answer"} else typ;_touch_relationship(con,network_id,owner,"uses" if typ=="service" else "contacts","device",str(owner),target,value,when,proto);relationships+=1
   owner=_traffic_owner(src_ip,dst_ip,src_did,dst_did)
   if owner is not None:
    remote=dst_ip if _local_ip(src_ip) else src_ip
    if _usable_host_ip(remote) and not _local_ip(remote):_touch_relationship(con,network_id,owner,"connects_to","device",str(owner),"ip",remote,when,proto);relationships+=1
